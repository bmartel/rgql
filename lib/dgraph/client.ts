...
export class DgraphClient {
  ...
  dgraph = async (query: string, variables?: any): Promise<any> => {
    try {
      ...
      const { data = {}, errors } = await (response.json() as Promise<{ data: any; errors: any }>)
      if (errors?.length) {
  ...
  getUserByAccount = async ({ provider, providerAccountId }: GetUserByAccountInput) => {
  ...
  updateSession = async (sessionToken: sessionId | unknown, input: UpdateSessionInput) => {
    const {
  ...
  getVerificationRequest = async ({ identifier, token }: any) => {
    const [verificationRequest] = await this.dgraph(queries.getVerificationRequest, {
      identifier,
      token,
    })
  ...
  deleteVerificationRequest = async ({ identifier, token }: DeleteVerificationInput) => {
    const {
